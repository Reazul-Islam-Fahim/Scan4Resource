...
# The same sanity check per kind of object: (width range, height range) in cm. Only these get a size.
LABEL_RANGES = {
    "door": (WIDTH_RANGE_CM, HEIGHT_RANGE_CM),
    "window": ((30.0, 350.0), (30.0, 300.0)),
}
...
def size_from_depth(depth: np.ndarray, box: Box, f_px: float, scale: float = 1.0,
                    ranges: Optional[tuple] = None) -> Optional[Observation]:
    """Width and height of the door (or window) in `box`, or None if this frame cannot give a trustworthy reading.

    `ranges` is ((min_w, max_w), (min_h, max_h)) in cm; the default is the door range."""
    width_range, height_range = ranges or LABEL_RANGES["door"]
    H, W = depth.shape
    ...
    if not (width_range[0] <= width_cm <= width_range[1] and height_range[0] <= height_cm <= height_range[1]):
        return None
...
class ReferenceMeasurer:
    ...
    def measure(self, ctx: FrameContext, box: Box, label: str = "door") -> Optional[Observation]:
        if label != "door":  # the standard-height assumption only holds for doors
            return None
...
class DepthMeasurer:
    ...
    def measure(self, ctx: FrameContext, box: Box, label: str = "door") -> Optional[Observation]:
        ranges = LABEL_RANGES.get(label)
        if ctx.depth is None or ranges is None:
            return None
        f_px = self.cfg.focal_ratio * max(ctx.width, ctx.height)
        return size_from_depth(ctx.depth, box, f_px, self.cfg.depth_scale, ranges)

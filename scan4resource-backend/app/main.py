"""Scan4Resource API, for the Scan4Resource frontend.
    ...
    POST   /detect       multipart: frame, scan_id, frame_index       -> { doors: [...], objects: [...] }
...
def create_app(settings: Optional[Settings] = None, pipeline: Optional[Pipeline] = None) -> FastAPI:
    ...
    def detect(
        ...
        """One camera frame in, what is visible in it out. `scan_id` ties the frames of one walkthrough
        together, so a door seen in many frames keeps one id. Materials are the keys wood, metal, glass, pvc,
        composite or unknown. `doors` is what the frontend has always read; `objects` holds everything else
        (window, chair, lamp, ...), each with a `label`."""
        try:
            image = decode_image(frame.file.read())
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        return app.state.pipeline.process_frame(image, scan_id or "default")

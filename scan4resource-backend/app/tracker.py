"""Count each object once: give the same physical door (or window, chair, ...) the same id across frames.
...
class SessionTracker:
    ...
    def __init__(self, cfg: Settings):
        ...
        self._next: dict[str, int] = {}  # per label, so ids read door-1, door-2, window-1, ...
    ...
    def update(self, detections: list[Detection], now: float) -> list[tuple[Track, Detection]]:
        ...
        for di, det in enumerate(detections):
            for ti, track in enumerate(active):
                if track.label != det.label:  # a door is never the same object as a window
                    continue
        ...
        for di, det in enumerate(detections):
            ...
            if track is not None:
                ...
                track = Track(self._new_id(det.label), det, now)
    ...
    def count(self, label: str) -> int:
        """Confirmed objects of one kind, e.g. count("door")."""
        return sum(1 for t in self.tracks if t.label == label and t.confirmed(self.cfg.min_hits))
